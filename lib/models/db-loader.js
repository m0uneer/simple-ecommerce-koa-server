...
module.exports = class DbLoader {
  ...
  static async init () {
    log.d('Loading NODE_CONFIG config', process.env.NODE_CONFIG_ENV, process.env.NODE_CONFIG_DIR)
    if (DbLoader.isInitiated) {
      return DbLoader.db
    }

    // Cloning dialectOptions as `sequelize@6.18.0` tries to edit it and `config` module prevents
    const dbOptions = { ...config, logging: dbLogger, dialectOptions: { ...config.dialectOptions } }
    log.d('Dialect', config.dialect)

    const sequelize = new Sequelize(config.url, dbOptions)
